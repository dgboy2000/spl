...
TODO: Above paragraph may need adjusting: this is plotted over outer iteration, but inner loop does a lot of iterations that we don't see here. Further, this behavior is tuned by a parameter $\mu$ for how quickly things anneal.  We may just have chosen criteria that require different values of $\mu$ to anneal at the same rate, e.g. maybe we'd better better results if we tuned $\mu$ to anneal faster on SPL.  Hump may depend just on how quickly we anneal, maybe there would be hump in inner loop?
...
TODO: I don't know about the `less of a departure' conclusion.  I'd explain the hamming distance results by talking about hidden variables, and how uncertainty focuses on getting the hidden variables right.  the hidden varaible is the motif location, so avg hamming dist starts small.  The other criteria are more agnostic to this.
...
We observe, at left, that slack and uncertainty select from similar pools of points in the first and second halves of their training, as revealed by the two rectangular blocks in the bottom left and top right of the plot.  Interestingly, the points seem to be uniformly scattered within each rectangle.  The
