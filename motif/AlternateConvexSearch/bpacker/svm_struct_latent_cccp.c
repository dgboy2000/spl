    ...
    /* add  constraint */
  	dXc = (DOC**)realloc(dXc, sizeof(DOC*)*size_active);
   	...
   	if (size_active>1) {
     	...
     	svm_model = (MODEL*)my_malloc(sizeof(MODEL)); 
     	svm_learn_optimization_slack(dXc,delta,size_active,sm->sizePsi,&lparm,&kparm,NULL,svm_model,alpha,cur_slack); 
   	} else {
   	...
   	value = margin - sprod_ns(w, new_constraint);
		...
		{
			printf("+"); fflush(stdout);
			size_active = resize_cleanup(size_active, &idle, &alpha, &delta, &dXc);
		}
